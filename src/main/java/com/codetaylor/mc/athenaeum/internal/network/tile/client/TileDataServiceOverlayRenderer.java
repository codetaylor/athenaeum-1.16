...
import com.codetaylor.mc.athenaeum.AthenaeumModClientConfig;
...
public class TileDataServiceOverlayRenderer {
  ...
  public static void onRenderGameOverlayPostEvent(RenderGameOverlayEvent.Post event) {
    ...
    if (!AthenaeumModClientConfig.enableTileDataServiceMonitor
    ...
    if (type == RenderGameOverlayEvent.ElementType.ALL) {
      ...
      INSTANCE.renderMonitor(matrixStack, TileDataServiceClientMonitor.TOTAL, scaledWidth / 2 - 32 - 128, 100, "Total Rx");

      // --- Position ---

      RayTraceResult traceResult = Minecraft.getInstance().objectMouseOver;

      if (traceResult != null
          && traceResult.getType() == RayTraceResult.Type.BLOCK) {

        BlockPos blockPos = ((BlockRayTraceResult) traceResult).getPos();
        ...
        if (monitor != null) {
          ...
          INSTANCE.renderMonitor(matrixStack, monitor, x, y, title);
        }
        ...
        if (map != null) {
          ...
          while (iterator.hasNext()) {
            ...
            fontRenderer.drawStringWithShadow(matrixStack, dataClass.getSimpleName() + " " + count, x + 64 /* TODO: const */ + 5, y + 9 + index * 10, Color.WHITE.getRGB());
            index += 1;
          }
        }
      }
  ...
  public void renderMonitor(MatrixStack matrixStack, TileDataServiceClientMonitor monitor, int x, int y, String title) {

    int trackedIndex = AthenaeumModClientConfig.tileDataServiceMonitorTrackedIndex;
...

    {
      ...
      fontRenderer.drawStringWithShadow(matrixStack, title, (float) (x - (textWidth / 2.0) + (totalWidth / 2.0)), y - 9, Color.WHITE.getRGB());
    }

    {
      String text = "§a" + min + " §e" + (int) (total / (float) size) + " §c" + max + " §9" + tracked;
      int textWidth = fontRenderer.getStringWidth(text);
      fontRenderer.drawStringWithShadow(matrixStack, text, (float) (x - (textWidth / 2.0) + (totalWidth / 2.0)), y, Color.WHITE.getRGB());
    }
